...
| DATABASE_TYPE               | Yes         |                   | Supports `Postgresql`, `Sqlite`, and `SqliteMemory`                                                                                                                         |
| DATABASE_URI                | Conditional |                   | Required when `DATABASE_TYPE` is `Postgresql`.<br/>Requires a virtual database to be selected; schema is optional. e.g. `postgresql://mydb:5432/corp?current_schema=public` |
...
| PORT                        | No          | `8000`            | Port the web service will bind to                                                                                                                                           |
| ADMIN_PORT                  | No          | `8001`            | Port the internal Admin API will bind to                                                                                                                                    |
| PASSWORD_AUTHENTICATION     | No          | `false`           | Prefer disabled in favour of Social Login                                                                                                                                   |
| BCRYPT_COST                 | No          | `12`              | Cost factor for BCrypt KDF operations. Permitted values: `4`, `8`, `10`, `12`, `14`, and `16`                                                                               |
| USER_SELF_REGISTRATION      | No          | `true`            | Require accounts to be provisioned by a super admin                                                                                                                         |
| APP_TITLE                   | No          | `Feature Control` | Friendly name for the application                                                                                                                                           |
| SESSION_LENGTH              | No          | `P7D`             | Length of a web session before it expires                                                                                                                                   |
| INVITATION_RETENTION_LENGTH | No          | `P7D`             | How long until a team invitation expires                                                                                                                                    |
| PAGE_SIZE                   | No          | `100`             | Internal page size for listing operation                                                                                                                                    |
| SDK_CACHE_MAX_AGE           | No          | `PT10S`           | `max-age` cache-control directive. Higher means greater cache efficiency at the risk of larger drift                                                                        |
| SDK_ACTIVE_TIMEOUT          | No          | `PT10M`           | How long without contact until an SDK is considered disconnected                                                                                                            |
| SUPER_ADMIN_EMAILS          | No          |                   | Entitles the users with the given email addresses to be super admins                                                                                                        |
